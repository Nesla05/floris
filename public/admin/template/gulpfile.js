...
gulp.task("injectLayoutStyles", function () {
  var verticalLightStream = gulp
    ...
    .pipe(gulp.dest("."));
  var horizontalLightStream = gulp
    .src("./**/horizontal-default-light/**/*.html")
    .pipe(
      inject(
        gulp.src(["./css/horizontal-layout-light/style.css"], { read: false }),
        { relative: true },
      ),
    )
    .pipe(gulp.dest("."));
  var horizontalDarkStream = gulp
    .src("./**/horizontal-default-dark/**/*.html")
    .pipe(
      inject(
        gulp.src(["./css/horizontal-layout-dark/style.css"], { read: false }),
        { relative: true },
      ),
    )
    .pipe(gulp.dest("."));
  var verticalDarkStream = gulp
    .src("./**/vertical-default-dark/**/*.html")
    .pipe(
      inject(
        gulp.src(["./css/vertical-layout-dark/style.css"], { read: false }),
        { relative: true },
      ),
    )
    .pipe(gulp.dest("."));
  return merge(
    verticalLightStream,
    horizontalLightStream,
    horizontalDarkStream,
    verticalDarkStream,
  );
});
...
gulp.task("copyAddonsScripts", function () {
  ...
  var aScript9 = gulp
    .src(["node_modules/jquery.avgrund/jquery.avgrund.min.js"])
